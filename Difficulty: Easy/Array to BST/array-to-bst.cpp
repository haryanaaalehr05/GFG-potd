...
class Solution {
  public:
    Node* vanshika(vector<int>& nums,int left,int right){
        //base case
        if(left>right){
            return NULL;
        }
        ...
        node->left=vanshika(nums,left,mid-1);
        node->right=vanshika(nums,mid+1,right);
    }
    
    Node* sortedArrayToBST(vector<int>& nums) {
        // Code here
        Node* root=vanshika(nums,0,nums.size()-1);
        return root;
    }
};
