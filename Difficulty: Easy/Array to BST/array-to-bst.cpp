...
class Solution {
  public:
    Node* loveneesh(vector<int>& nums,int left,int right){
        //base case
        if(left>right){
            return NULL;
        }
        ...
        node->left=loveneesh(nums,left,mid-1);
        node->right=loveneesh(nums,mid+1,right);
    }
    
    Node* sortedArrayToBST(vector<int>& nums) {
        // Code here
        Node* root=loveneesh(nums,0,nums.size()-1);
        return root;
    }
};
